...
Untouched content remains byte-for-byte unchanged. Missing or ambiguous selectors and invalid ranges fail before writing. A byte-identical result succeeds without rewriting the file. Results include a change summary and unified diff; output is capped at 200 lines or 8 KiB, with a truncation notice when needed. Use `read` to inspect resulting content beyond that preview.

...
Context pressure is the highest level reached by either relative or absolute usage: `medium` at 70% or 150k tokens, `high` at 80% or 200k, and `critical` at 90% or 250k. Lower usage is `low`; unavailable usage is `unknown`.
...
The `handoff_session` tool starts a parent-linked fresh session in TUI or RPC mode; both modes require a persisted Pi session. TUI uses a five-second cancellable countdown, while RPC continues automatically once the queued command runs. The tool accepts required `kickoff` text up to 16 KiB of UTF-8, preserves it exactly, delivers it as visible handoff context that triggers the replacement session, and transfers compatible extension continuity data such as timing. It only runs alone in its batch. If automatic delivery fails after replacement, the replacement editor receives the exact prepared text; if another extension cancels replacement before the switch, the original editor receives it for recovery. At session startup, pi-tools yields `handoff_session` ownership when another loaded extension already provides a tool with that name, avoiding conflicts with older awf project extensions.

The replacement starts without knowledge of the preceding conversation. Put every fact needed to continue in durable files or in the kickoff, and cite relevant files explicitly. Depending on the work, useful kickoff content includes the objective, current state, next action, decisions, constraints, completed work, verification, blockers, and unresolved questions. At medium pressure, do not hand off solely because of the pressure level: continue when retained context benefits the work and preserve important session-only knowledge for a possible later handoff. At high pressure, identify a safe handoff point and prepare continuity. At critical pressure, hand off as soon as safely possible.

A queued handoff suppresses only one imminent threshold-triggered compaction, allowing Pi to drain the queued continuation first. It also asks compatible remote-control extensions to suppress the current run's completion push, since work continues in the replacement session. If the agent calls `handoff_session` again while that continuation is pending, the retry refreshes the queued command, preserves the original kickoff, and returns a terminating success so the active run can settle instead of entering an error loop. Manual and overflow compactions remain available, and later threshold compactions are unaffected. Run `/reload` after updating either extension.

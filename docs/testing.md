...
`extensions/boundary-edit/replace-range.test.ts` checks literal selection and ambiguity (including overlapping occurrences), inclusive whole-line replacement, termination, EOF, mixed LF/CRLF, lone CR content, Unicode, and the current-content rather than snapshot contract. `index.test.ts` exercises temporary files, path handling, byte/BOM preservation, strict UTF-8 rejection, deletion, no-op write avoidance, pre-write cancellation, I/O errors, and bounds on both text and stored diff previews. It compares equivalent results with native `edit` and completed TUI rows through Pi's real `ToolExecutionComponent`, including diff colors, narrow widths, expansion, and theme invalidation. Pending, error, no-op, legacy text-only, and truncated-result rendering have focused assertions.

`tests/boundary-edit-runtime.test.ts` loads the manifest through isolated package settings and exercises the real `AgentSession` with a deterministic faux provider in print mode. It verifies discovery and execution, native-tool schemas/descriptions/provenance, and actual Pi error flags for invalid schema and selectors. A paused native edit checks that the loaded boundary tool cannot read until the native mutation finishes, including through a symlink alias, and that both unrelated changes survive. Only native I/O is gated; Pi's selection, mutation queue, and boundary tool execution stay real.

...
`extensions/boundary-edit/replace-range.test.ts` checks shared exact half-open resolution, all before/after combinations, inline/multiline/external context anchors, global ambiguity including overlapping occurrences, literal replacement, EOF, mixed LF/CRLF, lone CR and Unicode. Batched tests require original-file resolution independent of array order, shared context, disjoint/adjacent regions, overlap rejection, and insertion behavior. Red/green regressions cover duplicate insertions at a replacement's end and anchors that split a Unicode character. `feedback.test.ts` checks diagnostics, exact totals versus bounded candidate samples, merged context windows, reversed spans, physical line/UTF-8 byte counts, literal previews, total per-range line budgets, `all` beyond 200 lines, and the whole-response byte ceiling across huge lines/multiple ranges. A red/green regression requires a large first read to leave space for later diagnostics; unequal-sized ranges remain complete when the combined output fits. `index.test.ts` requires validation before writing, one write for a valid batch, per-entry outcomes, independent read results with multiple errors, byte/BOM preservation, strict UTF-8 rejection, no-op write avoidance, cancellation, partial-write warnings and editing after intervening changes. It exercises Pi's real `ToolExecutionComponent` for native-equivalent diff coloring, summary visibility at narrow widths, expansion/theme changes, pending/read/error/no-op/text-only/truncated output. Native edit's CRLF normalization is accounted for when comparing diff details; the boundary tool retains literal source endings.

`tests/boundary-edit-runtime.test.ts` loads the manifest through isolated package settings and exercises the real `AgentSession` with a deterministic faux provider in print mode. It verifies both tools' discovery, active system-prompt guidance and batch execution, absence of the removed tool, native-tool schemas/descriptions/provenance, and actual Pi error flags for invalid schema and selectors (including mixed read success/failure). A paused native edit checks that neither loaded boundary tool can read until the native mutation finishes, including through a symlink alias, and that reading preserves bytes while editing retains both unrelated changes. Only native I/O is gated; Pi's native edit, mutation queue, and boundary tool execution stay real.

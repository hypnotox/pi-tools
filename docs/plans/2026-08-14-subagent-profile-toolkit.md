...
- Terminal assurance added runtime validation for requested thinking levels, byte-bounded persisted execution facts and expanded profile rendering, complete child usage preservation plus parent tool-result accounting, and adversarial coverage. These fixes apply the existing callback-validation and bounded-observability decisions without changing the public profile boundary.

...
- Phase 2 review settlement made toolkit availability an uncorrelated capability announcement with correlated request replay, required stable consumer registration identifiers for idempotence, snapshotted and revalidated profile batches, finalized against configured active and inactive tools, removed discovered profile names inside marked children, and expanded package-level and protocol-matrix evidence. These corrections implement the reviewed load-order, atomicity, isolation, and typed-integration requirements without changing the approved boundary.

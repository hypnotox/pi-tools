...
- Phase 1 review settlement: populated ordered API-call recording, added harness contract tests and asynchronous factory readiness, replaced casted empty context services with public in-memory session state and inert callable UI/model surfaces, supplied fresh message-capable replacement contexts, confined handoff installation to the harness edge, restored its ownership-yield assertion, awaited lifecycle invocations, and added the Knip entry.
- Renewed Phase 1 settlement: replacement setup and callbacks now share one in-memory session, the inert theme supports every public formatting operation, fork joins replacement-context coverage, handoff delegates queue recording to the shared harness, and the generated testing document has one terminal newline. A temporary unused `testing/**` probe made Knip fail before removal; the restored tree passed Knip.
- Phase 2 migration inventory: context usage, runtime guard, subagent toolkit, and package subagent-profile integration now consume `pi-tools/testing`; subagent domain fixtures for profiles, registry, scheduler, runner, rendering, policy, deferred outcomes/event loop, model, subprocesses/streams, and schemas remain local. Focused evidence: the six-suite migration Vitest run and zero-match duplicate-scaffolding census passed; the SDK proof loaded an inline tool through public `DefaultResourceLoader`, `createAgentSession`, in-memory managers, public diagnostics, and public tool inspection without prompting, credentials, or network. Rendered prose inspection found the README, extension-toolchain topic, testing layout, decision index, and lockfile consistently describe complete adoption and a separate SDK proof. ADR operation disposition: add `development/extension-toolchain:reusable-extension-testing` Applied; no Remaining or Canceled operations.
- Reasoned deviation: `testing/index.ts` gained cohesive harness installation and shared event-bus injection because the remaining toolkit migrations require multiple factories and replacement runtimes at the same documented Pi-boundary recorder; verified by the migrated subagent lifecycle and replacement-bus tests.
- Phase 2 review settlement: restored direct runtime-guard boolean assertions, awaited every shared-harness installation, and protected the SDK proof's whole temporary tree from its first allocation. Temporarily changing its expected registered tool from `sdk_proof` to `sdk_missing` made the focused SDK test fail before restoration; the restored proof passed.
- Terminal assurance settlement: the package integration suite now saves, clears, and restores `PI_TOOLS_SUBAGENT_CHILD`, so its explicit parent-registration contract passes under both ordinary and child-marked test runners. The complete-range `./awf audit` passed with no findings. The generic `./x audit-local` lane is not configured in this repository, matching existing plan evidence; declared `./awf check` and `npm run check` gates passed. The terminal finding was mechanical, so no verify-pass review was required.
- Record further reasoned implementation deviations, review dispositions, migration inventories, focused test evidence, and rendered-prose inspection results here.

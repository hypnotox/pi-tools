...
1. `decision: cwd-relative-paths` In the child CWD line and activity summaries, including bash and
   custom summaries, render absolute paths within the parent Pi CWD as `./`-prefixed paths, render
   the CWD itself as `./`, and leave paths outside that root unchanged.
...
single-line constraint applies only to tool and retry activity entries. Truncation can hide
summary-tail details, so the full operation may require other evidence when those details matter.
Relativization remains a presentation concern, so persisted CWD and activity facts retain their
...
| Keep displayed paths absolute | Machine-specific prefixes consume terminal width without adding useful project-local context. |
...
- 2026-08-18: Implemented; content-sha256: a5dc5ac97a39886f03e64a63d122a040ea8dcd5e29023bc85660b6791457f62c

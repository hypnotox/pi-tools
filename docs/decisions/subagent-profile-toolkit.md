...
6. `decision: bounded-observability` Persist only final bounded JSON-serializable profile data with the toolkit's common execution facts. Use one toolkit-owned renderer for those facts, the final report or failure, and expanded bounded profile data. Rely on the child Pi process for transient request retries and never retry the whole subprocess.

...
export class ProfileRegistry {
  ...
  register(batch: ProfileRegistration): void {
    ...
    const suppressDefault = registrations.some((entry) => entry.suppressDefault);
    const proposed = [
      ...(suppressDefault ? [] : [this.#defaultProfile]),
      ...registrations.flatMap((entry) => entry.profiles),
    ];
  ...
  profileTools(): Set<string> {
    return new Set([
      ...
      ...this.#registrations.flatMap((batch) => batch.profiles.map((profile) => profile.toolName)),
    ]);
  }

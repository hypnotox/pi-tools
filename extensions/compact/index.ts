...
export default function compactExtension(pi: ExtensionAPI): void {
  ...
  registerContextOperation(pi, {
    ...
    description:
      "Compact context using Pi's native summarizer in the same session and runtime, then continue on success unless another continuation is observable through Pi's supported events/state. Allow the operation to finish before submitting new input; earlier async input hooks can hide an incoming prompt from continuation arbitration. Call this tool alone, without sibling tool calls, from a persisted TUI or RPC session. Supply nonblank multiline instructions of at most 16 KiB UTF-8 describing the objective, decisions, constraints, progress, references, unresolved work, and next action to preserve. These guide the summarizer; they are not an exact replacement summary or a verbatim-retention guarantee. Native compaction hooks still run; session-bound resources are not transferred or copied. A queued result is not completion. Cancellation and failure do not automatically retry or hand off.",
    randomUUID,
